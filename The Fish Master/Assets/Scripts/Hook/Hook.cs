...
public class Hook : MonoBehaviour {
    ...
    void Update()
    {
        ...
        Vector3 position = Vector3.zero;
            ...
            {
                vector = mainCamera.ScreenToWorldPoint(Input.mousePosition);
            ...
            #endif
            #if UNITY_ANDROID
            if (canMove && Input.GetTouch(0).phase == TouchPhase.Began)
            {
                vector = mainCamera.ScreenToWorldPoint(Input.GetTouch(0).position);
                position = transform.position;
                position.x = vector.x;
                transform.position = position;
            }
